 ...
 *
 *  - text over a surface axe declines to resolve. This palette builds its state
 *    tints out of EIGHT-DIGIT hex — `--surface-2: #16302810`,
 *    `--ok-tint: #10312592`, `--neutral-tint: #10283440` — which are
 *    translucent, so every panel painted with one shows whatever is behind it
 *    and axe reports `incomplete` rather than a ratio. The `<select>` is in
 *    that bucket on first paint.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This page is the strongest
 *    case of that in the fleet, and it is not decoration: the attack trace
 *    renders EVERY step at `opacity: 0.4` and the ladder renders every message
 *    at `opacity: 0.32` until the stepper reaches it, so the majority of the
 *    lab's headline output is faded most of the time. `.btn[disabled]` at `.55`
 *    is the third, and that one is an inactive component WCAG exempts.
 *
 ...
 *    boxes are `.trace` and `.knowledge` — both `max-height: 24rem` with
 *    `overflow: auto`, and both routinely longer than that — plus `.ladder`
 *    (`overflow: hidden`). Content scrolled past them is not dimmed or partly
 ...
 *    draws its one figure — the man-in-the-middle ladder — in CSS grid, not
 *    SVG, so the sibling walk finds nothing to do. It is kept because the
 *    alternative is a helper that silently mismeasures the first labelled
 *    diagram anyone adds: an ancestor-only walk would composite a `<text>` onto
 *    the `<svg>`'s own transparent background rather than onto the `<rect>`
 *    drawn under it, and report a ratio nothing on screen has. Its
 *    stroke-only guard is live regardless, on the shared header's hamburger.
 *
 ...
 * rule. `nontext.ts` measures the ones that carry a literal glyph.
 *
 * SECOND, `aria-hidden` subtrees, which BOTH oracles skip. See the `ariaHidden`
 * helper below for what this lab hides and why none of it carries a value.
 */
 ...
 * uses. The one genuinely short-lived rendering here is `animateSearch`, which
 * ticks the explored-state counter up over 750ms — and under the reduced motion
 * this gate asserts, `ui.ts` takes the `prefersReduced` branch and writes the
 * final number synchronously, so a scan cannot catch a half-counted state. The
 * parameter is kept because the narrowed form is what makes a short-lived state
 * measurable at all: a class that lives 500ms is shorter than a full axe pass,
 * so a whole-page scan would catch it only sometimes, and a sometimes-measured
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * that is `.trace` and `.knowledge` (`max-height: 24rem; overflow: auto`)
     * and `.ladder` (`overflow: hidden`). `.sr-only` is handled separately
     * below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // would apply the moment this lab grows an SVG figure of its own — the
      // ladder is CSS grid today.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // and slides in only on focus. Measuring the parked copy invents a
      ...
      // and `style.css` uses neither — this lab defines no `.sr-only` at all,
      // and its only off-screen text is the shared skip link, which parks by
      // POSITION and is caught by the guard above. The guard stays because the
      // clip idiom is the one that is invisible to `checkVisibility()`: such an
      // element keeps a 1x1 rect and reports itself visible while the
      // compositor draws zero pixels of it, so the day an `.sr-only` arrives
      // here a walk without this would report a fabricated ratio for it on
      // every single scan. It is deliberately narrow: only a ZERO-AREA clip
      // qualifies, so a partially clipped element is still measured.
     ...
     * decorative: the two `.ind-icon` glyphs on the verdict cards (🔒 and
     * ✓/✗/·), whose meaning is repeated verbatim in the `.ind-value` text
     * beside them in the same ink on the same surface, and the shared header's
     * hamburger and GitHub marks.
     *
     * Nothing on this page hides a VALUE. No trace step, no attacker fact, no
     * rule name, no state count and no verdict is inside an `aria-hidden`
     ...
     *
     * This lab has no SVG of its own — its one figure, the man-in-the-middle
     * ladder, is HTML and CSS grid. The guard stays because the usual way the mistake arrives is exactly
     ...
     * declares `background-color: var(--bg-solid)` explicitly, so the root's own
     * paint is what propagates to the canvas and the `<body>` fallback below is
     * never reached. The guard stays because the moment that declaration moves
     * to `body` with a `height: 100%` beside it, everything below the first
